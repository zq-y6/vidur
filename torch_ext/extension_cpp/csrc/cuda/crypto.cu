...
__constant__ __device__ uint cTe0_global[256] = {
...
__constant__ __device__ uint cTe1_global[256] = {
...
__constant__ __device__ uint cTe2_global[256] = {
...
__constant__ __device__ uint cTe3_global[256] = {
...
__constant__ __device__ uint cTe4_global[256] = {
...
__global__ void aes_encrypt_kernel(const uint *pt, uint *ct, const uint *iv, const uint *rek, uint Nr) {
  __shared__ uint cTe0[256];
  __shared__ uint cTe1[256];
  __shared__ uint cTe2[256];
  __shared__ uint cTe3[256];
  __shared__ uint cTe4[256];

  int x = blockIdx.x * blockDim.x + threadIdx.x;
  int y = blockIdx.y * blockDim.y + threadIdx.y;

  int index = x + y * gridDim.x * blockDim.x; /* The block index */
  int offset = index << 2;  /* Offset in the plaintext */

  // printf("block index is %d\n", index);

  if (threadIdx.x == 0) {
    for (int i = 0; i < 256; i++) {
      cTe0[i] = cTe0_global[i];
      cTe1[i] = cTe1_global[i];
      cTe2[i] = cTe2_global[i];
      cTe3[i] = cTe3_global[i];
      cTe4[i] = cTe4_global[i];
    }
  }
  __syncthreads();

  // Seed and Round Key Generation
  // https://xilinx.github.io/Vitis_Libraries/security/2021.1/guide_L1/internals/aes.html
  uint seed[4];
  ...
  // Encryption
  // https://github.com/cbguder/aes-on-cuda
  ...
  // XOR
  ct[offset + 0] = pt[offset + 0] ^ seed[0];
  ct[offset + 1] = pt[offset + 1] ^ seed[1];
  ct[offset + 2] = pt[offset + 2] ^ seed[2];
  ct[offset + 3] = pt[offset + 3] ^ seed[3];
}

at::Tensor my_paralell_aes_encrypt_cuda(const at::Tensor& pt, at::Tensor& ct, const at::Tensor& iv, const at::Tensor& rek) {
  TORCH_CHECK(pt.nbytes() % 16 == 0 && pt.nbytes() != 0);   // 128-bit (block size) aligned plaintext
  TORCH_CHECK(rek.nbytes() == 224);   // 256-bit key --> key expansion --> 56*4 bytes round key
  TORCH_CHECK(iv.nbytes() == 12);     // 12-bit iv
  TORCH_CHECK(pt.nbytes() == ct.nbytes());
  TORCH_CHECK(pt.is_contiguous());    // should be cont
  TORCH_CHECK(ct.is_contiguous());    // should be cont
  TORCH_CHECK(iv.is_contiguous());    // should be cont
  TORCH_CHECK(rek.is_contiguous());   // should be cont
  TORCH_CHECK(pt.dtype() == at::kUInt32);
  TORCH_CHECK(ct.dtype() == at::kUInt32);
  TORCH_CHECK(iv.dtype() == at::kUInt32);
  TORCH_CHECK(rek.dtype() == at::kUInt32);
  TORCH_INTERNAL_ASSERT(pt.device().type() == at::DeviceType::CUDA);
  TORCH_INTERNAL_ASSERT(ct.device().type() == at::DeviceType::CUDA);
  TORCH_INTERNAL_ASSERT(iv.device().type() == at::DeviceType::CUDA);
  TORCH_INTERNAL_ASSERT(rek.device().type() == at::DeviceType::CUDA);
  const uint* pt_ptr = pt.data_ptr<uint>();   // Pytorch will check nullptr internally when calling the data_ptr() method
  const uint* iv_ptr = iv.data_ptr<uint>();
  const uint* rek_ptr = rek.data_ptr<uint>();
  uint* ct_ptr = ct.data_ptr<uint>();
  const int max_threads_per_block = 256;
  int total = pt.nbytes() / 16;
  ...
  aes_encrypt_kernel<<<blocks, threads>>>(pt_ptr, ct_ptr, iv_ptr, rek_ptr, 14);
  return ct;
}
